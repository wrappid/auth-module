...
const API_URL = "https://demo.wrappid.dev/";
...
describe("login", () => {
  ...
  test("TC02 Verify API Response Body Structure", async () => {
    ...
    expect(response.body).toHaveProperty("user"); // Assuming a user object is returned in the response
  });

  test("TC03 Verify API Response with Invalid Credentials", async () => {
    const response = await request(API_URL)
      .post("/login")
      .send({ emailOrPhone: "invalid@email.com", password: "wrongpassword" })
      .set("Content-Type", "application/json")
      .set("Accept-Encoding", "gzip, deflate, br")
      .set("Connection", "keep-alive")
      .set("User-Agent",
    ...
    expect(response.statusCode).toBe(401); // Assuming 401 is the unauthorized status code
    expect(response.body).toHaveProperty("error");
  });

  // New Test Cases
  test("TC04 Verify API Response with Missing Email/Phone", async () => {
    const response = await request(API_URL)
      .post("/login")
    ...
    expect(response.statusCode).toBe(400); // Assuming 400 is the bad request status code
    expect(response.body).toHaveProperty("error"); // Expect error message for missing field
  });
